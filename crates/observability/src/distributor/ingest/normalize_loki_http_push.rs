use super::{
    DistributorError, HeaderMap, Limits, LokiProtoPushRequest, Message, SnappyDecoder, TenantId,
    Value, WalLogRecord, decode_loki_http_body, is_loki_json_content_type,
...
pub(crate) fn normalize_loki_http_push(
    ...
    if is_loki_json_content_type(headers)? {
        ...
        normalize_loki_push(tenant, payload, limits)
    } else {
        ...
        normalize_loki_proto_push(tenant, payload, limits)
    }
}
