use super::{
    ALL_OPS, CancellationToken, DistributorState, JoinHandle, ObjectStore, Router, ServiceConfig,
    ServiceConfigError, ServiceDependencies, ServiceMetrics, compactor_delete_requests_for_config,
    delete_request_routes, distributor_push_routes, querier_routes_with_shutdown,
...
pub(crate) async fn all_in_one_router(
    ...
    let (querier_routes, background_tasks) = querier_routes_with_shutdown(
        config,
        dependencies
        ...
        readiness.for_role(RoleKind::Querier),
    )
    .await?;
