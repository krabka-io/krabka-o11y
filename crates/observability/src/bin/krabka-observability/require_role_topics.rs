...
/// is no topic to check and nothing to refuse.
///
/// # Errors
/// Returns [`TopicContractError`] when no bootstrap address answers, or when a
/// topic is absent or not compacted.
pub(crate) async fn require_role_topics(config: &ServiceConfig) -> Result<(), TopicContractError> {
    let Some(bootstrap) = config.wal_bootstrap_server.as_deref() else {
        return Ok(());
    };
    require_topics(bootstrap, &LOGS_TOPICS).await?;
    Ok(())
}
