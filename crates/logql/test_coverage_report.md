# krabka-logql Test Coverage Report

| Document Info | Details |
| :--- | :--- |
| **Crate** | `krabka-logql` |
| **Signal** | logs |
| **Upstream surface** | LogQL parsing, formatting, filtering, and planning |
| **Date** | 2026-09-13 |

## Compatibility Coverage Summary

The owned behavior below has executable coverage, and delegated compatibility is stated as a gap.

| Surface | Behavior | Result | Test | Oracle |
| :--- | :--- | :--- | :--- | :--- |
| **LogQL parsing, formatting, filtering, and planning** | Arbitrary parsed expressions format and parse again | Pass | `tests/parser_proptest.rs::formatting_a_logql_expression_round_trips` | Property invariant |

## Test Inventory

The crate has 281 source-declared unit, property, corpus, or integration tests.

Run the complete non-container inventory with:

```bash
cargo test --package krabka-logql --locked -- --list
```

Tests under `tests/` cover public crate boundaries, while tests under `src/` cover local behavior.

Docker-tagged differential suites are listed in the root [compatibility matrix](../../docs/api_compatibility.md).

## Coverage vs Scope

| Area | Scenario | Planned | Implemented | Status |
| :--- | :--- | :--- | :--- | :--- |
| Owned surface | Arbitrary parsed expressions format and parse again | 1 | 1 | Complete |
| Delegated or external surface | Live Loki response compatibility is delegated to krabka-observability. | — | — | Delegated or excluded |
| **Total owned** |  | **1** | **1** | **100%** |

## Line Coverage

Run the scoped measurement with:

```bash
cargo llvm-cov nextest --package krabka-logql --profile ci --lib --bins --lcov --output-path lcov.info
lcov --summary lcov.info
```

| File | Covered | Total | Coverage | Notes |
| :--- | :--- | :--- | :--- | :--- |
| Scoped crate |  |  |  | Fill from the command above; CI also uploads workspace LCOV to Codecov |

The scoped command excludes ignored container suites.

## Mutation Coverage

The scheduled target is `bazel test //crates/logql:logql_mutants`.

Its reviewed baseline is 91 survivors from a complete 12-shard run. The exact
commit, command, runner, duration, and artifact checksums are recorded in
[`qualification/milestone-19-mutation-baselines.json`](../../qualification/milestone-19-mutation-baselines.json).

## Test Infrastructure

Tests use `assert2`, in-memory trait implementations, and checked-in fixtures where those seams apply.

Property and corpus harnesses run as ordinary Bazel and Cargo tests, and live upstream comparisons carry the `docker` tag.

## Key Gaps

| Area | Gap | Severity | Notes |
| :--- | :--- | :--- | :--- |
| Coverage measurement | No checked-in scoped line percentage | Low | Generate it with the command above |
| Compatibility boundary | Live Loki response compatibility is delegated to krabka-observability. | Low | The owning crate or Docker suite carries the claim |

## Conclusion

The report accounts for 281 source-declared tests and one representative owned behavior at 100% scope coverage.

Line and mutation percentages remain unquoted until their complete tool output is available.
