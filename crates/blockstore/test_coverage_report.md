# krabka-blockstore Test Coverage Report

| Document Info | Details |
| :--- | :--- |
| **Crate** | `krabka-blockstore` |
| **Signal** | all |
| **Upstream surface** | Parquet blocks, indexes, compaction, retention, and tenant paths |
| **Date** | 2026-09-13 |
...
| :--- | :--- | :--- | :--- | :--- |
| **Parquet blocks, indexes, compaction, retention, and tenant paths** | Block write and read round trips | Pass | `tests/roundtrip_proptest.rs::write_then_read_preserves_all_rows` | Property invariant |

## Test Inventory

The crate has 374 source-declared unit, property, corpus, or integration tests.

Run the complete non-container inventory with:

```bash
cargo test --package krabka-blockstore --locked -- --list
```
...
| :--- | :--- | :--- | :--- | :--- |
| Owned surface | Block write and read round trips | 1 | 1 | Complete |
| Delegated or external surface | Container-backed object-store failure modes remain outside the scoped line run. | — | — | Delegated or excluded |
| **Total owned** |  | **1** | **1** | **100%** |
...
```bash
cargo llvm-cov nextest --package krabka-blockstore --profile ci --lib --bins --lcov --output-path lcov.info
...
The scheduled target is `bazel test //crates/blockstore:blockstore_mutants`.
...
| Compatibility boundary | Container-backed object-store failure modes remain outside the scoped line run. | Low | The owning crate or Docker suite carries the claim |

## Conclusion

The report accounts for 374 source-declared tests and one representative owned behavior at 100% scope coverage.
