...
/// Lists every object owned by the index stored at `key` that lies under
/// `listed_prefix`.
///
/// # Errors
///
...
pub async fn list_index_object_keys(
    ...
    if Path::from(key).prefix_matches(listed_prefix) {
    ...
    if Path::from(snapshot_prefix.as_str()).prefix_matches(listed_prefix) {
    ...
    if Path::from(shards_prefix.as_str()).prefix_matches(listed_prefix) {
    ...
    if Path::from(payload_prefix.as_str()).prefix_matches(listed_prefix) {
...
mod tests {
    ...
    async fn ignores_block_subtrees_that_share_index_prefix_names() {
        ...
        check!(
            list_index_object_keys(&store, key, &Path::from(""))
                .await
                .expect("list keys")
                == valid.into()
        );
    ...
    #[tokio::test]
    async fn extensionless_index_siblings_outside_the_listed_prefix_are_skipped() {
        let store: Arc<dyn ObjectStore> = Arc::new(object_store::memory::InMemory::new());
        let key = "traces";
        let objects = [
            key.to_string(),
        ...
        for object in &objects {
            store
                .put(
                    &Path::from(object.as_str()),
                    PutPayload::from_static(b"object"),
                )
                .await
                .expect("write object");
        }
        ...
        check!(
            list_index_object_keys(&store, key, &Path::from("traces"))
                .await
                .expect("list keys")
