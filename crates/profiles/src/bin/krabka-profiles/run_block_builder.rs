...
use super::{CancellationToken, Cli, ServiceMetrics, block_builder_config, build_object_store};
...
pub(crate) async fn run_block_builder(
    ...
    let config = block_builder_config(&cli, configured.store, index_key, metrics);
