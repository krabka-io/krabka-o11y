...
const MIMIR_AGREES_WITH_KRABKA: &[KnownDivergence] = &[KnownDivergence {
    reason: "An empty native histogram renders as `\"buckets\": []` on Mimir 2.16.1, exactly as \
             Krabka renders it. Omitting the field is newer than the Prometheus that Mimir \
             embeds, so the shared divergence does not show here.",
    cases: &[
        "functions.test:1075",
        "functions.test:1078",
        "functions.test:1081",
