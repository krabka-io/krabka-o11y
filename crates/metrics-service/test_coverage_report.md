# krabka-metrics-service Test Coverage Report

| Document Info | Details |
| :--- | :--- |
| **Crate** | `krabka-metrics-service` |
| **Signal** | metrics |
| **Upstream surface** | Prometheus and Mimir query APIs, query frontend, and ruler |
| **Date** | 2026-09-13 |
...
| :--- | :--- | :--- | :--- | :--- |
| **Prometheus and Mimir query APIs, query frontend, and ruler** | PromQL corpus results match Prometheus | Pass | `tests/diff_prometheus.rs::prometheus_compliance_corpus_matches_krabka` | Pinned Prometheus image |

## Test Inventory

The crate has 114 source-declared unit, property, corpus, or integration tests.

Run the complete non-container inventory with:

```bash
cargo test --package krabka-metrics-service --locked -- --list
```
...
| :--- | :--- | :--- | :--- | :--- |
| Owned surface | PromQL corpus results match Prometheus | 1 | 1 | Complete |
| Delegated or external surface | Ignored differential suites require Docker and are excluded from the scoped line run. | — | — | Delegated or excluded |
| **Total owned** |  | **1** | **1** | **100%** |
...
```bash
cargo llvm-cov nextest --package krabka-metrics-service --profile ci --lib --bins --lcov --output-path lcov.info
...
The scheduled target is `bazel test //crates/metrics-service:metrics-service_mutants`.
...
| Compatibility boundary | Ignored differential suites require Docker and are excluded from the scoped line run. | Low | The owning crate or Docker suite carries the claim |

## Conclusion

The report accounts for 114 source-declared tests and one representative owned behavior at 100% scope coverage.
