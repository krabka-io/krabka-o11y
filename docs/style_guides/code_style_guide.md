...
- Nightly has two sanctioned tool uses. `cargo +nightly fmt` formats options that are still nightly-gated, and `cargo +nightly fuzz` runs cargo-fuzz under `fuzz/`. Crate language features must remain on the pinned stable toolchain.
